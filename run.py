...
class TradingEngine:
    ...
    def refresh_markets(self):
        try:
            ...
            for m in markets:
                # KRITICKÉ: Posíláme subscribe jen pokud ID skutečně nemáme v aktivních odběrech
                if m.condition_id not in self.active_subscriptions:
                    mins_left = (m.end_time - now).total_seconds() / 60
                    if mins_left < 1.0: continue # Ignorujeme trhy co končí hned
                    ...
                    logging.info(f"📡 Subscribing to NEW market: {m.asset}")

            # Vyčištění starých ID
            self.active_subscriptions = {sid for sid in self.active_subscriptions if sid in new_ids}
            
        except Exception as e:
            logging.error(f"⚠️ Refresh Skip: {e}")
    ...
    async def decision_loop(self):
        while self.running:
            try:
                ...
                # Prodlouženo na 300s (5 minut), abychom nedostali 1008 Policy Violation
                if not self.markets or (now_ts - self.last_market_refresh > 300):
                ...
                for cid, m in list(self.markets.items()):
                    ...
                    # Pokud data o orderbooku jsou None, nepanikaříme, jen čekáme
    ...
    def _update_dashboard_only(self):
        ...
        try:
            update_dashboard_state(
                ...
                markets={cid: {'asset': m.asset, 'prob': self.states[cid].prob, 'time_left': 5} for cid, m in self.markets.items()}
            )
        except: pass
